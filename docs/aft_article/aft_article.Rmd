...
Instead of enumerating important steps in computational folkloristics \cite{abello_computational_2012} in a chronological manner, for the sake of brevity we present related progress in the context of *evolving semantics*. In interaction with vector space based methods and predication based semantic indexing \cite{cohen_discovering_2012}, this paradigm is emerging e.g. in digital preservation \cite{kontopoulos2016pericles}, \cite{kontopoulos2016deliverable}. 

## Converging trends

In this broader context, one  can observe two major trends whose convergence will be underlying the results of the next decade. The first is focus on the evolutionary aspect of motif and/or tale type distributions, either with regard to certain tale types \cite{silva_comparative_2016}, \cite{karsdorp_topic_2013}, \cite{karsdorp_retelling_2016} (eg d’Huy and Tehrani), or geographical distribution of globally occurring narrative motifs \cite{thuillard_large-scale_2018}. Strikingly, there is a certain genetically inspired thinking in the background, ultimately going back to the modeling capacities inherent in Dawkins' meme theory \cite{dawkins_selfish}, comparing tale types as motif sequences to 'narrative DNA'\cite{daranyi_toward_2012}, \cite{ofek_linking_2013}, \cite{meder_automatic_2016} (Murphy 2014, 2015), or looking at the evolution of narrative/story networks as a quasi-biological process based on the mutation and recombination of narrative elements \cite{karsdorp_retelling_2016}. Such views possibly rely on certain similarities with bioinformatics in terms of network motif identification (ref), a problem analog with ours. 
The second popular trend is that of computing embeddings by multivariate statistics; this means geometrically located content of increasingly condensed word and sentence semantics and beyond \cite{mikolov2013efficient}, \cite{pennington-etal-2014-glove}, \cite{rothe-schutze-2015-autoextend}, \cite{DBLP:journals/corr/LeM14}, \cite{reimers2019sentencebert}, \cite{NEURIPS2019_cb12d7f9}, and must be familiar from information retrieval, machine learning, data science or knowledge representation. Many results of computational folkloristics manifest examples of this paradigm \cite{tangherlini_bigfolk}, \cite{lo_exploring_2020}. Related methods fall back on *distributional semantics*. However, vector spaces are not really suitable to investigate semantic evolution per se, the notion asking for vector fields instead to model the inherent dynamics of content \cite{wittek_monitoring_2015}, \cite{daranyi_physical_2016}. Unfortunately, no semantic theory is available to explain factors behind language change or conceptual dynamics \cite{daranyi_demonstrating_2013} in terms of vector fields for the time being.

Another method of encoding sentence semantics, reliant on *compositional semantics*, connects to quantum theory (QT) inspired text processing methods, a research direction in artificial intelligence \cite{DBLP:journals/corr/abs-2101-04255}. The first publications looking at the structural study of Greek mythology from a QT perspective were published a while ago \cite{daranyi_sphynxs_2014}, \cite{daranyi_conceptual_2016}, expected to pave the way for similar efforts. 
