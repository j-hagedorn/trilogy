...
- `provenance` field is still messy, since multiple variables (i.e. country, region, tale collection) are still stored in a single column
...
- *Open data*: In order to use tale data consistently, it must be made freely and openly available to anyone. 
...
by_type %>%
  ggplot(aes(x = fct_reorder(atu_id,n_tales), y = n_tales)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Number of tales, by tale type",
    x = "ATU Tale Type"
  ) +
  theme_bar_flip
...
average number of text words in a tale, perhaps by type 
