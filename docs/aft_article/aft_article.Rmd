---
title: Bearing a bag-of-tales`:`
subtitle: An open corpus of annotated folktales for reproducible research
titlerunning: Bearing a bag-of-tales
authorrunning: Hagedorn, J. & Darányi, S.
thanks: | 
...
- name: Joshua Hagedorn
  address: Independent researcher, Grand Rapids MI, USA
...
- Other researchers who have shared their data as supporting material for their articles include e.g. \cite{tehrani_oral_2016}, \cite{silva_comparative_2016}, \cite{tehrani_riding_2013} and \cite{bortolini_inferring_2017}. Also, \cite{declerck_linked_2017} and \cite{declerck_ontologies_2017} report that recently, a large amount of ATU data have been made available online by the Multilingual Folk Tale Database^[https://www.mftd.org], offering also annotation facilities for tales in multilingual versions. A step in the right direction as it sounds, as this paper goes to press, we have no information about the conditions of their usability yet.
...
1. Navigate to the [dataset's URL](https://github.com/j-hagedorn/trilogy/blob/master/data/aft.csv).
2. Press the `y` button on the keyboard to get a permanent link to the exact version of the dataset.^[See the GitHub documentation on "[Getting permanent links to files](https://docs.github.com/en/github/managing-files-in-a-repository/getting-permanent-links-to-files)" for more information.]  Even as the dataset changes, other researchers will be able to use this link to run their code against the same version you used.
...
## Growing and Refining the Corpus {#grow}

Existing lists of motifs and tale types are incomplete \cite{dhuy_unbiased_2017}, and thus any corpus of texts is also bound to be incomplete if its contents are limited to tales annotated using these lists.
...
A new set of annotated tales can be submitted via a "pull request" in *GitHub*, which would include a script to fetch new texts and transform them into the dataframe structure used by the `aft` dataset^[Similar to the `fetch_ashliman.R` script used to obtain the initial dataset.], as well as any source files required to run the script.  Pull requests provide a structure for submission of changes, as well as for testing and review of newly introduced code.  Collaborators are encouraged to publish descriptions of their contributions as well: for instance, as a short data paper to the *Journal of Open Humanities Data*.
