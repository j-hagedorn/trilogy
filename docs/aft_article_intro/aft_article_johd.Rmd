...
```{r dpi=300,fig.retina=1,fig.width= 8}
library(huxtable)
showtext_auto()
...
ggsave("wordhist.png", units="in", width=5, height=2, dpi=200)
...
by_type <- 
  ...
  summarize(
    n_tales = n(),
    avg_tokens_tale = round(mean(n_tokens), digits = 1)
  )
...
by_type %>%
  ...
  mutate(
    label = paste0(
      if_else(
        is.na(word(tale_name,1,5)),
        tale_name,
        word(tale_name,1,5)
      ),
      " (",atu_id,")"
    ),
    label = fct_reorder(label, n_tales)
  ) %>%
...
ggsave("lollipop.png", units="in", width=5, height=3, dpi=200)
...
t <-
  ...
  set_font(font_family) %>%
  set_align(-1, 2, ".") %>%
  set_caption("Ten tale types with the largest number of representative tales")
```
